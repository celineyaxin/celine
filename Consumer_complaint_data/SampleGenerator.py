...
class ComplaintDataAugmentor:
    def __init__(self, api_key):
        ...
        # 保留我们调整好的专业prompt
        self.prompt_template = """作为行为金融学专家，请基于您已学习的代表性偏差分类标准，为以下投诉文本生成3个变体：
...
### 生成要求
基于您对代表性偏差的深刻理解，生成3个变体，每个必须：
- 保持相同的代表性偏差类型（从原始文本中识别出的类型）
- 改变具体表述但保持核心认知偏差机制
- 确保变体自然真实，符合投诉场景
    ...
    def augment_complaints(self, input_file, output_file=None, 
        ...
        try:
            ...
            if output_file is None:
                ...
                if max_rows:
                    output_file = f"{base_name}_数据增强_前{max_rows}条.xlsx"
                else:
                    output_file = f"{base_name}_数据增强_全部.xlsx"
...
def main():
    ...
    # 输入文件路径 - 请修改为您的实际文件路径
    input_file = "/Users/chenyaxin/Desktop/审稿修改/分类数据/计算能力/满足偏差的文本.xlsx"
    ...
    try:
        augmentor = ComplaintDataAugmentor(API_KEY)
