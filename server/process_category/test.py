...
if __name__ == "__main__":
    ...
    print(f"抽取前 {TEST_SAMPLE_SIZE} 行进行测试")
    ...
    print(f"时间筛选后剩余 {len(limited_df)} 行")
    ...
    # 构造带有日期范围的输出CSV文件名
    output_csv_path = os.path.join(news_classifier.save_path, f'TEST_financial_predictions_{start_date_str}_{end_date_str}.csv')
    limited_df.to_csv(output_csv_path, index=False)
    print(f"测试完成！Predictions saved to {output_csv_path}")