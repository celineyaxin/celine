...
keywords = ['银行', '基金', '教育','贷','健身','整形','医院','医美','金融','手游','驾校','火锅','房地产','家居','潮玩','玩具','拼多多','手表','中介','生物','轻奢','厨具',
            ...
            '借','财经','财会','相机','钱包','保险','会计','学校','医生','床垫','冰箱','支付','运动','影音','保健','男装','图书','门窗','出租车','专营店','照片','鞋','轻断食',
            '学校','学堂','瑜伽','网教','人寿','面包','同仁堂','体育','方特','坚果','维修','复印','体脂','资产','贸易','散热','租赁','法律','电玩','商城','牛奶',
            '短视频','快递','乐园','小说','人保寿险','婚纱','公主','知识','手机','学历','餐饮','安防','药房','家具','4s','家具','纹绣','科技发展','店','租房','咨询','加油',
            '祛痘','钢琴','英语','数码','汽车','皮肤','家纺','服饰','资产管理','地产','设计师','珠宝','护肤','雅思','电竞','摩托车','摄像头','生鲜','理财','生活','蛋糕','橱柜',
            '斗地主','pos','婚恋','医疗','充电宝','顺风车','家电','家居','快跑','水果','奶茶','健康','燕窝','首饰','电器','营销','卫浴','电子商务','数字科技','闲置','法考',
            '早教','充电','文学','植发','信息科技','成语','直播','万科','照明','摄影','食品','工具','美妆','分期','车饰','万达','旗舰店','批发','优选','疫苗','传播','影视',
            ...
            '派送','人力资源','房产','印刷','单车','女装','考研','婚姻','空调','窗帘','房产','财险','物联网','农业','区块链','广告','口腔','饮食','建材','咖啡','影城','电影',
            '化妆品','女包','盲盒','车险','体检','搬家','新能源','酸奶','ETC','etc','数据恢复','特卖','药业','眼科','投影','TV','tv','录屏','减肥','购书','单词','影业','交友',
            '法务','彩票','交友','恋爱','木门','足球','寻爱','相亲','考试','租车','旅拍','出租','天气','口腔','书吧','建材','全屋定制','木门','担保','日语','自主洗','产后修复',
            '闪送','瓷砖','洗碗机','小吃','整装','集成灶','厨卫','鲜花','翻译','辅导','厨房','戏剧','茶叶','代驾','食谱','橱柜','轮胎','财税','净水器','净水器','地板','装潢',
            '股票','月子中心','美语']
...
mask_keywords = filtered_df['投诉商家'].str.contains(regex_pattern, na=False, case=False)
...
df_filtered = filtered_df[~mask_keywords & ~filtered_df['投诉商家'].isin(business_names) & ~filtered_df['投诉商家'].isin(financial_business_names)]
df_rejected = filtered_df[mask_keywords | filtered_df['投诉商家'].isin(business_names) | filtered_df['投诉商家'].isin(financial_business_names)]
