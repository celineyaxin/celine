...
fraud_df = pd.read_csv(
    '/Users/chenyaxin/Desktop/InternetTourismConvention/data/travel_complaints.csv')

# 2. 读取合并结果并提取投诉编号
merged_df = pd.read_excel('/Users/chenyaxin/Desktop/travel_sample_dp.xlsx')
...
unmatched_fraud = unmatched_fraud.drop_duplicates(
    subset=['投诉编号'], keep='first')
print(f"未匹配的欺诈投诉数量: {len(unmatched_fraud)}")

# 5. 从投诉时间提取年份信息
# 假设'投诉时间'列包含日期字符串，如"2023-05-15"
unmatched_fraud.loc[:, '年份'] = pd.to_datetime(unmatched_fraud['发布时间'], format='%Y年%m月%d日 %H:%M').dt.year
# 6. 确保年份和商家分类列存在
if '年份' not in unmatched_fraud.columns:
    raise ValueError("数据集中缺少'年份'列")
if '投诉对象' not in unmatched_fraud.columns:
    raise ValueError("数据集中缺少'投诉对象'列")
...
def stratified_sample(df, strata_columns, sample_size):
    ...
    分层抽样函数
    :param df: 数据框
    :param strata_columns: 分层列名列表，如['年份', '投诉对象']
...
# 检查商家分类分布
print("\n商家分类分布:")
print(supplement_sample['投诉对象'].value_counts())

# 10. 保存结果
output_path = '/Users/chenyaxin/Desktop/supplement_travel_sample.csv'
supplement_sample.to_csv(output_path, index=False)
print(f"\n补充样本已保存至: {output_path}")

# 11. 如果需要，合并所有欺诈样本
final_fraud_set = pd.concat(
    [matched_fraud, supplement_sample], ignore_index=True)
