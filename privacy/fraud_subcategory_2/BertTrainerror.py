...
class NewsClassifier:
    ...
    def predict(self, text):
        self.model.eval()
        input_ids = self.tokenizer(text, padding='max_length', max_length=512, truncation=True, return_tensors="pt").input_ids
        input_ids = input_ids.to(self.device)
        attention_mask = self.tokenizer(text, padding='max_length', max_length=512, truncation=True, return_tensors="pt").attention_mask
...
if __name__ == "__main__":
    ...
    news_classifier.train(epochs=10, batch_size=32, lr=5e-5)
    # news_classifier.train(epochs=10, batch_size=64, lr=1e-5)
    # 评估测试集
    ...
    dev_df = news_classifier.load_data(data_paths['dev'])
