...
input_file = '/Users/chenyaxin/Desktop/business_classify.xlsx'
...
# 1. 合并稀有类别（样本数少于3的类别）
min_samples = 3
class_counts = df['子分类'].value_counts()
...
if rare_classes:
    ...
    df['adjusted_class'] = df['子分类'].apply(
        lambda x: '其他' if x in rare_classes else str(x)
    )
else:
    df['adjusted_class'] = df['子分类'].astype(str)
