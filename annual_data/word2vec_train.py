...
def word2vec_train(infile,outmodel,outvector,size,window,min_count):
    ...
    # train model
    # model = Word2Vec(LineSentence(infile),vector_size=size,window=window,min_count=min_count,workers=multiprocessing.cpu_count())
    with open(infile, 'r', encoding='utf-8') as f:
        data = json.load(f)
    model = Word2Vec(data,vector_size=size,window=window,min_count=min_count,workers=multiprocessing.cpu_count())
    # save model
    model.save(outmodel)
