...
output_excel_file = (
    "/Users/chenyaxin/Desktop/recruitment_nokeywordqw.xlsx"  # 输出Excel文件名
)
...
2. 大数据工程技术人员：大数据采集、清洗、分析、治理、挖掘、系统开发与大数据运维
...
9. 商务数据分析师：商务数据采集、清洗、挖掘、分析，指标体系构建，数据建模，分析报告。**仅当其工作内容包含公司内部业务数据（如销售、用户、运营数据）的分析，而非仅专注于宏观行业与市场数据分析时，方属于本类别。**
10. 电子商务师：仅当明确包含"数据采集和销售数据分析"（指公司内部的销售数据）或"商务网站数据架构设计"时。**仅处理外部行业市场数据、不涉及本公司业务数据分析的岗位不属于本类别。**
11.成本测算相关岗位：明确出具数据分析报告时


【不属于1的情况（应归类为0） - 必须严格遵守】
*   **通用职能岗位**：如行政、人事、财务、法务、普通文员等。
*   **传统销售、市场、运营、客服类岗位**：**此为重要排除项。** 工作核心目标是以达成销售交易、客户关系维护、品牌宣传、市场活动策划、用户运营、客户服务为主的岗位。即使职位描述中提及“数据分析”，但如果其仅是要求**简单使用Excel、PPT或第三方工具（如CRM、BI看板）进行报表制作、数据查看、结果描述或汇报**，而**非**将其作为核心专业技能进行数据采集、处理、建模、挖掘或系统开发，则不属于本类别。
...
*   **科学研究与实验分析岗位**：即使工作描述涉及数据分析并与“统计专业人员”的任务有表面重合，但如果其核心任务是分析以下数据类型：实验数据（如生物实验数据、化学检测数据、材料物理性能数据、心理学实验数据等）、财务数据（库存管理，产供销环节，销售额等传统财务指标数据）、自然数据（地理，气象数据），且旨在得出科学结论而非构建数据系统或提供通用商业决策支持，则不属于本类别。
*   **其他**：工作描述中完全不含数据处理、分析、管理或相关系统开发、算法应用等关键词的岗位。
*   **新增排除项**：工作内容**仅涉及**收集、分析宏观**行业数据、市场调研数据、舆情数据**，而完全不涉及企业自身业务数据（如销售、用户行为、产品运营等）的处理、建模与分析，其产出物仅为市场研究报告或行业洞察报告的岗位。
【增强判断逻辑】
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "你是一位专业的金融数据分析研究员，正在研究企业数据密集度。",
                    },
                ...
                if response_text:
                    try:
                        ...
                        if json_start >= 0 and json_end > json_start:
                            ...
                            classification_result = response_json.get(
                                "classification_result", 0
                            )
                            ...
                            if (
                                ...
                                or "true" in response_text.lower()
                            ):
...
print(f"\n分类结果统计:")
print(
    f"数据处理相关岗位: {data_related_count} ({data_related_count/total_count*100:.2f}%)"
)
print(
    f"非数据处理岗位: {total_count - data_related_count} ({(total_count - data_related_count)/total_count*100:.2f}%)"
)
