...
def load_previous_sample(previous_sample_file):
    """
    加载之前抽样的数据分析职位样本
    
    参数:
    previous_sample_file: 之前抽样的CSV文件路径
    
    返回:
    包含已抽样职位描述的DataFrame
    """
    previous_sample = pd.read_csv(previous_sample_file)
    return previous_sample

def exclude_previous_sample(original_df, previous_sample_df, job_desc_column):
    """
    从原始数据中排除已抽样的职位
    
    参数:
    original_df: 原始数据的DataFrame
    previous_sample_df: 已抽样数据的DataFrame
    ...
    """
    # 获取已抽样职位的职位描述
    sampled_descriptions = set(previous_sample_df[job_desc_column].astype(str))
    
    # 创建掩码，排除已抽样的职位
    mask = ~original_df[job_desc_column].astype(str).isin(sampled_descriptions)
    filtered_df = original_df[mask].copy()
    
    print(f"原始数据行数: {len(original_df)}")
    print(f"排除已抽样职位后行数: {len(filtered_df)}")
...
def main():
    ...
    previous_sample_file = "/Users/chenyaxin/Desktop/上市公司投诉数据探究/招聘数据处理/分类/通过关键词抽样分类/数据分析职位样本.csv"  # 之前抽样的数据分析职位样本
    ...
    try:
        ...
        # 步骤2: 读取之前抽样的数据分析职位样本
        print("正在读取之前抽样的数据分析职位样本...")
        previous_sample_df = load_previous_sample(previous_sample_file)
        
        # 步骤3: 从原始数据中排除已抽样的职位
        print("正在排除已抽样的数据分析职位...")
        non_analysis_df = exclude_previous_sample(original_df, previous_sample_df, job_description_column)
        
        # 步骤4: 去除职位描述重复的行
        print("正在去除重复的职位描述...")
        unique_jobs = remove_duplicate_job_descriptions(non_analysis_df, job_description_column)
        ...
        # 保存结果
        output_file = "/Users/chenyaxin/Desktop/非数据分析职位样本.csv"
