...
def balance_mechanism_categories(file_path, output_path):
    """
    平衡三个机制类别的数据集，同时考虑商家和年份分布
    ...
    # 应用映射
    df['机制类别'] = df['分类结果_qwen'].apply(
        lambda x: next((k for k, v in mechanism_mapping.items() if x in v), '未知')
    )
    
    # 删除未知类别的样本
    df = df[df['机制类别'] != '未知']
    print(f"删除未知类别后数据条数: {len(df)}")
    ...
    for mechanism, target_count in target_counts.items():
        # 获取该类别的数据
        mechanism_df = df[df['机制类别'] == mechanism].copy()
        current_count = len(mechanism_df)
        ...
        if current_count > target_count:
            # 需要下采样
            balanced_df = stratified_downsample(mechanism_df, target_count)
            print(f"下采样到: {len(balanced_df)}")
        else:
            # 数量不足或正好，保持原样
            balanced_df = mechanism_df
            print(f"保持原数量: {current_count}")
        
        # 显示采样后的分布情况
        if len(balanced_df) > 0:
            # 显示原始子类别分布
            subcategory_dist = balanced_df['分类结果_qwen'].value_counts()
            print(f"原始子类别分布: {dict(subcategory_dist)}")
    ...
    # 统计最终分布
    final_mechanism_dist = final_df['机制类别'].value_counts()
    print(f"\n最终机制类别分布:")
    for mechanism, count in final_mechanism_dist.items():
        percentage = count / len(final_df) * 100
        print(f"{mechanism}: {count} 条 ({percentage:.1f}%)")
    
    final_subcategory_dist = final_df['分类结果_qwen'].value_counts()
    print(f"\n最终原始子类别分布:")
    for subcategory, count in final_subcategory_dist.items():
        percentage = count / len(final_df) * 100
        print(f"{subcategory}: {count} 条 ({percentage:.1f}%)")
...
if __name__ == "__main__":
    ...
    output_file = "/Users/chenyaxin/Desktop/信息规范/文本分类/合并类别数据_平衡后.xlsx"
    
    try:
        balanced_data = balance_mechanism_categories(input_file, output_file)
        print("数据处理完成！")
