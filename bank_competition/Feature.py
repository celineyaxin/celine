...
def extract_features_from_files(csv_folder, target_complaint_ids):
    ...
    返回一个字典：{投诉编号: [涉诉金额, 投诉进度, 商家回复时间1, 发起投诉时间]}
    """
    ...
    for i, file_path in enumerate(csv_files, 1):
        ...
        try:
            ...
            chunk_iter = pd.read_csv(
                file_path, 
                usecols=['投诉编号', '涉诉金额', '投诉进度', '商家回复时间1', '发起投诉时间'],
                chunksize=50000,
                dtype={'投诉编号': str}
            )
...
def merge_features_to_original(original_df, features_dict, original_order):
    ...
    if '投诉编号' in original_df.columns and len(original_df.columns) > 1:
        ...
        result_df['涉诉金额'] = features_df['涉诉金额']
        result_df['投诉进度'] = features_df['投诉进度']
        result_df['商家回复时间1'] = features_df['商家回复时间1']
        result_df['发起投诉时间'] = features_df['发起投诉时间']  # 新增的列
    else:
        # 如果原始数据只有投诉编号，直接创建新DataFrame
        result_df = pd.DataFrame({
            '投诉编号': original_order,
            '涉诉金额': features_df['涉诉金额'],
            '投诉进度': features_df['投诉进度'],
            '商家回复时间1': features_df['商家回复时间1'],
            '发起投诉时间': features_df['发起投诉时间']  # 新增的列
        })
...
def efficient_data_processing(csv_folder, complaint_ids_file, output_file):
    ...
    # 输出各列的匹配情况
    for col in ['涉诉金额', '投诉进度', '商家回复时间1', '发起投诉时间']:
...
if __name__ == "__main__":
    ...
    # csv_folder = "/Volumes/T9/test"
    complaint_ids_file = "/Users/chenyaxin/Desktop/信息规范/data/投诉内容汇总.csv"
    output_file = "/Users/chenyaxin/Desktop/信息规范/data/complaints_feature.csv"
