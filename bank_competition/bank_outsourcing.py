...
def analyze_complaint_text(complaint_text):
    """
    分析单条投诉内容是否与银行外包业务相关，特别关注信息科技外包风险
    ...
    # 构建针对外包业务分析的提示词，强调信息科技外包
    prompt = f"""
您是一名资深银行合规与风险管理专家，请分析以下银行客户投诉内容，判断是否涉及银行的外包业务，特别是信息科技外包（根据银保监会通知要求）。
...
2.  **是否涉及信息科技外包**（重点）：
    - 是否涉及信息技术服务，如软件开发、系统维护、数据处理、云计算、网络安全等？
    - 是否符合《银行保险机构信息科技外包风险监管办法》中定义的外包活动？

3.  **信息科技外包风险等级**（如适用）：
    - 低风险：一般操作问题，无安全或合规风险。
    - 中风险：涉及潜在数据泄露或系统故障，但未造成实际损失。
    - 高风险：确认的数据泄露、系统中断、违规操作等，可能影响业务连续性或客户数据安全。
...
8.  **关键理由**：
    - 简要解释判断的依据，引用文本中的关键描述，特别是与信息科技外包风险相关的部分。
...
"""
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "你是一名严谨的银行合规分析专家，擅长从投诉内容中识别外包业务相关问题，特别关注信息科技外包风险。请严格按照用户要求的格式输出结果。"},
...
def parse_analysis_result(result_text):
    ...
    try:
        ...
        it_outsourcing_match = re.search(r"是否涉及信息科技外包[：:]\s*【?(是|否|不确定)】?", result_text)
        risk_level_match = re.search(r"信息科技外包风险等级[：:]\s*【?(低|中|高|无)】?", result_text)
        ...
        if it_outsourcing_match:
            parsed_result["是否涉及信息科技外包"] = it_outsourcing_match.group(1)
        if risk_level_match:
            parsed_result["信息科技外包风险等级"] = risk_level_match.group(1)
...
def process_csv_file(file_path, output_path):
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            df.at[index, "是否涉及信息科技外包"] = result["是否涉及信息科技外包"]
            df.at[index, "信息科技外包风险等级"] = result["信息科技外包风险等级"]
        ...
        it_outsourcing_count = len(df[df["是否涉及信息科技外包"] == "是"])
        uncertain_count = len(df[df["是否涉及外包业务"] == "不确定"])
        
        print(f"\n分析统计:")
        print(f"- 涉及外包业务的投诉: {outsourcing_count}")
        print(f"- 涉及信息科技外包的投诉: {it_outsourcing_count}")
        ...
        # 如果有涉及信息科技外包的投诉，打印风险等级分布
        if it_outsourcing_count > 0:
            print("\n信息科技外包风险等级分布:")
            risk_levels = df[df["是否涉及信息科技外包"] == "是"]["信息科技外包风险等级"].value_counts()
