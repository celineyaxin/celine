...
# 定义文件路径
input_file = '/Users/chenyaxin/Desktop/信息规范/文本分类/平衡后的数据集.xlsx'
...
# 选择需要的列
df = df[['发起投诉内容', '分类结果_qwen']]

# 重命名列，使其更清晰
df = df.rename(columns={'发起投诉内容': 'text', '分类结果_qwen': 'label'})
...
# 使用分层抽样划分数据集 - 更合理的比例
# 先划分训练集和临时集（测试+验证），训练集占85%
train_df, temp_df = train_test_split(
    df, 
    test_size=0.2, 
    random_state=42,
    stratify=df['label']  # 按类别分层
)
...
# 保存标签映射关系为txt文件（便于查看）
label_mapping_file = '/Users/chenyaxin/Desktop/label_mapping.txt'
with open(label_mapping_file, 'w', encoding='utf-8') as f:
    ...
    f.write(f"\n总计: {len(label_encoder.classes_)} 个类别\n")
    f.write(f"生成时间: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

print(f"\n标签映射已保存到: {label_mapping_file}")
