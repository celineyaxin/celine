...
def main():
    ...
    output_file = "/Users/chenyaxin/Desktop/行政处罚分类结果_测试100条.xlsx"  # 修改输出文件名
    ...
    try:
        # 读取数据但只取前100条
        df = pd.read_excel(input_file)
        test_df = df.head(100).copy()  # 只取前100条
        
        # 保存测试数据到临时文件
        temp_input_file = "/Users/chenyaxin/Desktop/测试数据_100条.xlsx"
        test_df.to_excel(temp_input_file, index=False)
        print(f"已创建测试数据文件: {temp_input_file}")
        
        # 分批处理数据（使用临时文件）
        final_result = process_data_in_batches(
            input_file=temp_input_file,  # 使用测试数据文件
            output_file=output_file,
            text_column='主要违法违规事实',  # 根据实际列名调整
            batch_size=10,
            delay=1,
            start_from=0
        )
